 ...
 *
 * When repositoryId is provided, the format is: `{prefix}:{repositoryId}:{qualifiedName}`
 * When repositoryId is not provided (backward compatibility): `{prefix}:{qualifiedName}`
 *
 ...
 * @param repositoryId - Optional repository UUID for multi-repository support.
 * @returns The generated entity ID string.
 */
export function generateEntityId(prefix: string, qualifiedName: string, repositoryId?: string): string {
    ...
    // Include repositoryId in the entityId for multi-repository support
    if (repositoryId) {
        const safeRepoId = repositoryId.toLowerCase().replace(/[^a-z0-9-]/g, '_');
        return `${normalizedPrefix}:${safeRepoId}:${safeIdentifier}`;
    }

    return `${normalizedPrefix}:${safeIdentifier}`; // Backward compatible format
}
