...
export class PythonAstParser {
    ...
    constructor(pythonExecutable?: string) {
        this.pythonExecutable = pythonExecutable || config.pythonExecutable;
        this.validatePythonExecutable();
        logger.debug(`Python AST Parser initialized with executable: ${this.pythonExecutable}`);
    }

    /**
     * Validates and potentially fallback to alternative Python executables.
     * Tries in order: configured executable -> python3 -> python
     */
    private validatePythonExecutable(): void {
        const candidates = [this.pythonExecutable, 'python3', 'python'];
        ...
        logger.warn(`No valid Python executable found. Tried: ${candidates.join(', ')}`);
        logger.warn(`Will attempt to use configured executable: ${this.pythonExecutable}`);
    }

    /**
     * Checks if a Python executable is available and working.
     */
    private isPythonExecutableValid(executable: string): boolean {
        try {
            execSync(`${executable} --version`, { stdio: 'ignore', timeout: 5000 });
            return true;
        } catch {
            return false;
        }
